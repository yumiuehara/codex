...
export default function SeriesPage() {
  ...
  const seriesList = logList.filter((log) => {
    const start = String(getYearFromCustomDate(log.startDate));

    let end = start
    if (log.endDate) end = String(getYearFromCustomDate(log.endDate));

    return (start === params.year || end === params.year) && log.media.idMedia === MediaTypeEnum.TV_SERIES;
  });

  return (
    <FullListLayout title={t("misc.series")}>
      <ListSection data={seriesList} customClass="mx-10 my-5" />
    </FullListLayout>
  );
}
