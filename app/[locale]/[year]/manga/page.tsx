...
export default function MangaPage() {
  ...
  const mangaList = logList.filter((log) => {
    const start = String(getYearFromCustomDate(log.startDate));

    let end = start
    if (log.endDate) end = String(getYearFromCustomDate(log.endDate));

    return (start === params.year || end === params.year) && log.media.idMedia === MediaTypeEnum.MANGA;
  });

  return (
    <FullListLayout title={t("misc.manga")}>
      <ListSection data={mangaList} customClass="mx-10 my-5" />
    </FullListLayout>
  );
}
